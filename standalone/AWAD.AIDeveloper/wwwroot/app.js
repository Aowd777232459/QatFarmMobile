...
function requireWorkspace(){if(!currentWorkspace){toast('افتح مشروعًا أولاً');return false}return true}
...
async function loadTree(){
  try{
    ...
    const tree=$('#tree');
    tree.innerHTML=r.tree.map(x=>`<button class="tree-item" data-path="${esc(x.path)}" title="فتح ${esc(x.path)}">${esc(x.path)}</button>`).join('')||'<div class="muted">المجلد فارغ.</div>';
    tree.querySelectorAll('.tree-item').forEach(b=>b.onclick=()=>openFile(b.dataset.path));
  }catch(e){$('#tree').innerHTML=`<div class="muted">${esc(e.message)}</div>`}
}
function renderPlan(plan){
  currentPlan=plan;$('#planStatus').textContent=plan.status==='done'?'جاهز للتنفيذ':plan.status==='blocked'?'يحتاج معلومات':'خطة مرحلية';
  $('#reviewBanner').className='review-banner hidden';$('#reviewBanner').innerHTML='';
...
async function openFile(path){
  ...
  try{busy(true,'جاري فتح الملف...');const r=await api('/api/file/read',{method:'POST',body:JSON.stringify({path})});currentFilePath=r.path;$('#editorPath').textContent=r.path;$('#editorContent').value=r.content;modal('editorModal')}catch(e){toast(e.message)}finally{busy(false)}
}
...
$('#planBtn').onclick=async()=>{const msg=$('#prompt').value.trim();if(!msg)return toast('اكتب المهمة أولاً');if(!requireWorkspace())return;appendMessage('user',msg);try{busy(true,'الذكاء الاصطناعي يحلل المشروع ويجهز الخطة...');const p=await api('/api/chat',{method:'POST',body:JSON.stringify({message:msg})});renderPlan(p);appendMessage('assistant',`${p.summary}\n${p.next||''}`);setLog('تم إنشاء الخطة. لم يتم تعديل أي ملف بعد.')}catch(e){appendMessage('assistant','تعذر إنشاء الخطة: '+e.message);toast(e.message)}finally{busy(false)}};
...
$('#discardBtn').onclick=()=>{currentPlan=null;$('#applyBar').classList.add('hidden');$('#reviewBanner').className='review-banner hidden';$('#planArea').className='plan-area empty-state';$('#planArea').innerHTML='<div class="empty-icon">⌘</div><strong>تم إلغاء الخطة</strong><span>يمكنك كتابة طلب جديد.</span>';$('#planStatus').textContent='لا توجد خطة'};

$('#previewBtn').onclick=async()=>{if(!currentPlan)return;try{busy(true,'جاري حساب الفرق قبل التنفيذ...');const list=await api('/api/preview',{method:'POST',body:JSON.stringify({planId:currentPlan.id})});$('#previewList').innerHTML=list.length?list.map(x=>`<div class="preview-item"><div class="preview-head"><strong>${esc(x.path)}</strong><span>${esc(x.operation)}</span></div><pre class="diff-view">${esc(x.diff)}</pre></div>`).join(''):'<div class="empty-state">لا توجد تغييرات نصية لمعاينتها.</div>';modal('previewModal')}catch(e){toast(e.message)}finally{busy(false)}};
$('#reviewBtn').onclick=async()=>{if(!currentPlan)return;try{busy(true,'المراجع الذكي يفحص الخطة بشكل مستقل...');const r=await api('/api/review',{method:'POST',body:JSON.stringify({planId:currentPlan.id})});const box=$('#reviewBanner');box.className=`review-banner ${r.approved?'review-ok':'review-warn'}`;box.innerHTML=`<div class="review-score"><b>${r.score}/100</b><span>${r.approved?'موافق':'يحتاج مراجعة'}</span></div><div><strong>${esc(r.summary)}</strong>${r.risks?.length?`<p><b>المخاطر:</b> ${r.risks.map(esc).join(' · ')}</p>`:''}${r.suggestions?.length?`<p><b>الاقتراحات:</b> ${r.suggestions.map(esc).join(' · ')}</p>`:''}</div>`;appendMessage('assistant',`نتيجة المراجعة المستقلة: ${r.score}/100 — ${r.summary}`);toast(r.approved?'المراجع وافق على الخطة':'المراجع وجد نقاطًا تحتاج انتباهًا')}catch(e){toast(e.message)}finally{busy(false)}};

$('#autoBtn').onclick=async()=>{const msg=$('#prompt').value.trim();if(!msg)return toast('اكتب المهمة أولاً');if(!requireWorkspace())return;if(!confirm('الوضع التلقائي سيطبق التعديلات ويشغّل أوامر البناء المسموحة داخل المشروع. سيتم إنشاء نقاط استعادة. هل تريد المتابعة؟'))return;appendMessage('user',msg);try{busy(true,'جاري التطوير التلقائي واختبار النتيجة...');setLog('بدأت دورة التطوير التلقائي...');const r=await api('/api/auto',{method:'POST',body:JSON.stringify({message:msg,maxCycles:Number($('#cycles').value||3)})});let log=[r.message];for(const c of r.cycles||[]){log.push(`\n=== CYCLE ${c.cycle} ===\n${c.plan.summary}`);for(const l of c.apply.logs||[])log.push(`${l.success?'[OK]':'[FAIL]'} ${l.type} ${l.target}\n${l.message}`)}setLog(log.join('\n'));appendMessage('assistant',r.message);if(r.cycles?.length)renderPlan(r.cycles[r.cycles.length-1].plan);await loadTree();toast(r.success?'اكتملت المهمة':'توقفت بعد الحد الأقصى للدورات')}catch(e){appendMessage('assistant','فشل التطوير التلقائي: '+e.message);setLog('ERROR: '+e.message);toast(e.message)}finally{busy(false)}};
...
$('#saveFileBtn').onclick=async()=>{if(!currentFilePath)return;try{busy(true,'جاري حفظ الملف وإنشاء نسخة احتياطية...');const r=await api('/api/file/save',{method:'POST',body:JSON.stringify({path:currentFilePath,content:$('#editorContent').value})});toast('تم الحفظ مع checkpoint احتياطي');setLog(`تم حفظ ${r.path}\nCheckpoint: ${r.checkpoint}`);await loadTree()}catch(e){toast(e.message)}finally{busy(false)}};

$('#terminalBtn').onclick=()=>{if(requireWorkspace())modal('terminalModal')};
$('#runTerminalBtn').onclick=async()=>{const command=$('#terminalCommand').value.trim();if(!command)return toast('اكتب الأمر');try{busy(true,'جاري تشغيل الأمر داخل المشروع...');const r=await api('/api/terminal',{method:'POST',body:JSON.stringify({command})});$('#terminalOutput').textContent=`ExitCode: ${r.exitCode}\n\n${r.output||'(no output)'}`;toast(r.exitCode===0?'اكتمل الأمر بنجاح':'انتهى الأمر بخطأ')}catch(e){$('#terminalOutput').textContent='ERROR: '+e.message;toast(e.message)}finally{busy(false)}};
$('#terminalCommand').addEventListener('keydown',e=>{if(e.key==='Enter')$('#runTerminalBtn').click()});

async function loadGit(){if(!requireWorkspace())return;try{busy(true,'جاري قراءة حالة Git...');const r=await api('/api/git');$('#gitBranch').textContent=r.isRepository?`الفرع: ${r.branch||'غير معروف'}`:'ليس مستودع Git';$('#gitStatus').textContent=r.status||'لا توجد تغييرات.';$('#gitDiff').textContent=r.diffStat||'لا يوجد diff.';$('#gitLog').textContent=r.recentCommits||'لا يوجد سجل.';$('#gitCheckpointBtn').disabled=!r.isRepository;modal('gitModal')}catch(e){toast(e.message)}finally{busy(false)}}
$('#gitBtn').onclick=loadGit;
$('#refreshGitBtn').onclick=loadGit;
$('#gitCheckpointBtn').onclick=async()=>{if(!confirm('سيتم تنفيذ git add -A ثم إنشاء commit محلي. لن يتم push إلى الإنترنت. متابعة؟'))return;try{busy(true,'جاري إنشاء Git checkpoint...');const r=await api('/api/git/checkpoint',{method:'POST',body:JSON.stringify({message:$('#gitMessage').value})});toast(r.exitCode===0?'تم إنشاء Git checkpoint':'تعذر إنشاء checkpoint');await loadGit()}catch(e){toast(e.message)}finally{busy(false)}};
